...
export class FiltroComponent {
  ...
  async ngOnInit(){
    ...
    this.cursos = workbook.SheetNames;
    workbook.SheetNames.forEach(wkb => {
      ...
      raw_data.slice(5).forEach((r:any) => {      
    ...
    this.horarios.forEach((element) => {      
      for(var i = 0; i < 6; i++){
        ...
        dias.forEach((day : String) => {    
          if(semana.indexOf(day.toLowerCase()) == i && element.curso.toLowerCase() == this.selected.curso.toLowerCase() && element.semestre.toLowerCase() == this.selected.semestre.toLowerCase()){
            var dt1 = new Date(); 
            var dt2 = new Date();
            var dtI = new Date();  
            var dtF = new Date(); 
            ...
            for(var j = 0; j < 29; j++){
              ...
              if((dtI.toLocaleTimeString() >= dt1.toLocaleTimeString() && dtI.toLocaleTimeString() < dt2.toLocaleTimeString()) || (dtI.toLocaleTimeString() < dt1.toLocaleTimeString() && dtF.toLocaleTimeString() > dt2.toLocaleTimeString())|| (dtI.toLocaleTimeString() < dt1.toLocaleTimeString() && dtF.toLocaleTimeString() == dt2.toLocaleTimeString())){           
                ...
                if(periodo!=""){
                  let data = new Date(parseInt(this.selected.hoje.split("-")[0]),parseInt(this.selected.hoje.split("-")[1])-1,parseInt(this.selected.hoje.split("-")[2]));
                  if((dtI <= data && dtF >= data)){
